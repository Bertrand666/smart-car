...
#include "stm32f10x_usart.h"            // 添加USART头文件
#include "stm32f10x_gpio.h"             // 添加GPIO头文件
#include "stm32f10x_rcc.h"              // 添加RCC头文件
#include "misc.h"                       // 添加misc.h以解决中断向量定义问题
...
{
	static uint8_t RxState = 0;		// 串口接收状态机状态：0-等待包头，1-接收数据，2-等待包尾
	static uint8_t pRxPacket = 0;	// 当前接收数据包中的位置索引
...
// 外部函数声明
extern void HCSR04_Follow_Start(void);
...
void spp_control(void)
{
	...
	{
		// 不再直接显示蓝牙指令，避免与模式显示冲突
		// OLED_ShowString(4, 1, "                ");
		// OLED_ShowString(4, 1, Serial_RxPacket);
		
		/*将收到的数据包与预设的指令对比，以此决定将要执行的操作*/
		if (strcmp(Serial_RxPacket, "Car_Forward") == 0)			// 直行
		{
			Car_Forward(300);
		}
		else if (strcmp(Serial_RxPacket, "Car_Backward") == 0)		// 倒车
		{
			Car_Backward(300);
		}
		else if (strcmp(Serial_RxPacket, "Car_TurnLeft") == 0)		// 左转
		{
			Car_SpinLeft(300);
			Delay_ms(700);
			Car_Forward(300);
		}
		else if (strcmp(Serial_RxPacket, "Car_TurnRight") == 0)		// 右转
		{
			Car_SpinRight(300);
			Delay_ms(700);
			Car_Forward(300);
		}
		else if (strcmp(Serial_RxPacket, "Car_SpinLeft") == 0)	  	// 原地左转
		{
			Car_SpinLeft(300);
		}
		else if (strcmp(Serial_RxPacket, "Car_SpinRight") == 0)		// 原地右转
		{
			Car_SpinRight(300);
		}
		else if (strcmp(Serial_RxPacket, "Car_Stop") == 0)	     	// 停车
		{
			Car_Stop();
		}
		// 模式切换命令
